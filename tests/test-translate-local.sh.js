...
for (const forbidden of ['--force', 'push origin main', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY']) {
...
for (const key of ['TRANSLATE_LOCAL_REEXEC', 'TRANSLATE_CHUNK_SELFTEST', 'TRANSLATE_TRANSLATOR', 'TRANSLATE_MODEL', 'TRANSLATE_EFFORT']) delete env[key];
...
function run(config = {}, args = [], overrides = {}) {
  ...
  const result = spawnSync('/bin/bash', [copy, ...args], { env: { ...env, ...overrides }, encoding: 'utf8', timeout: 15000 });
...
try {
  ...
  const chunked = spawnSync(script, [packets], { env: { ...env, TRANSLATE_CHUNK_SELFTEST: '1' }, encoding: 'utf8', timeout: 5000 });
...
`.replace('@NODE@', process.execPath);
  ...
  assert.ok(!result.calls.some(c => ['codex', 'claude'].includes(c.tool)));
  result = run({ fail: true }, ['--max-per-call', '1', '--base', 'stack', '--translator', 'codex', '--model', 'chosen', '--effort', 'high'], { TRANSLATE_TRANSLATOR: 'claude', TRANSLATE_MODEL: 'env-model', TRANSLATE_EFFORT: 'low' });
  assert.equal(result.status, 0, result.stderr);
  assert.equal(result.calls.filter(c => c.tool === 'codex').length, 3);
  assert.ok(result.calls.filter(c => c.tool === 'codex').every(c => c.cwd === wt && c.args.includes('chosen') && c.args.includes('model_reasoning_effort="high"')));
  assert.match(fs.readFileSync(path.join(wt, '.translate/pr-body.md'), 'utf8'), /Applied 1 strings for 1 locales \(bb\).*review pass ran: yes\.[\s\S]*REPORT[\s\S]*QUALITY[\s\S]*Errors:   0[\s\S]*Warnings: 2[\s\S]*Generated by scripts\/translate-local.sh/);
  ...
  fs.writeFileSync(callsFile, '');
  const locked = spawnSync('/bin/bash', ['-c', 'exec 9>"$1"; /usr/bin/lockf -s -t 0 9; /bin/bash "$2" 9>&-', 'test', path.join(root, '.claude/auto-translate.lock'), copy], { env, encoding: 'utf8', timeout: 5000 });
